...
double *read_matrix_from_file(char *fname, int nrows, int ncols);
...
int main(int argc, char* argv[])
{
  ...
  if (argc > 1) {
    ...
    if (myid == master) {
      if(argc > 3){
	aa = read_matrix_from_file(argv[2], nrows, ncols);
	bb = read_matrix_from_file(argv[3], nrows, ncols);
      } else {
...
double *read_matrix_from_file(char *fname, int nrows, int ncols)
{
  ...
  double *c = (double *) malloc(nrows * ncols * sizeof(double));
  double val;
